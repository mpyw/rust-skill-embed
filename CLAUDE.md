...
| One directory reached by two agent paths | A repository can link `.claude/skills` at `.agents/skills`, so that every agent reads one tree. `targets` merges destinations by path, so the two spellings are two of them. Each skill is written twice and listed twice. Both writes carry the same bytes, and the run after them reads up to date at both. Merging by identity would print one destination, under whichever spelling the agent order reached first. That is `.agents/skills`, so the path the reader linked would appear nowhere. It also does not reach the one case that misleads: `uninstall --agent claude-code` takes the skill from every agent sharing the tree, and that run has one target either way. `one_directory_reached_by_two_agent_paths` pins it |
